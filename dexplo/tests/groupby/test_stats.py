...
class TestValueCounts:
    ...
    def test_value_counts(self):
        ...
        df2 = de.DataFrame({'AIRLINE': ['DL', 'AA', 'AS', 'WN', 'B6', 'UA', 'VX', 'EV'],
                            'count': [2, 2, 1, 1, 1, 1, 1, 1]})
        assert_frame_equal(df1, df2)

        df1 = df.value_counts('DAY_OF_WEEK')
        df2 = de.DataFrame({'DAY_OF_WEEK': [5, 4, 3, 1, 7, 6, 2], 'count': [3, 2, 2, 2, 1, 1, 1]})
        assert_frame_equal(df1, df2)

        df1 = df.value_counts('DEPARTURE_DELAY')
        df2 = de.DataFrame({'DEPARTURE_DELAY': [22.0, -1.0, -4.0, -2.0, 21.0, 3.0, -3.0],
                            'count': [2, 2, 2, 1, 1, 1, 1]})
        assert_frame_equal(df1, df2)
        ...
        df2 = de.DataFrame({'DAY_OF_WEEK': [5, 4, 3, 1, 7, 6, 2],
                            'count': [0.25,
                                      0.16666666666666666,
                                      0.16666666666666666,
                                      0.16666666666666666,
    ...
    def test_value_counts_sort_na(self):
        ...
        df2 = de.DataFrame({'DAY_OF_WEEK': [1, 2, 3, 4, 5, 6, 7], 'count': [2, 1, 2, 2, 3, 1, 1]})
        assert_frame_equal(df1, df2)

        df1 = df.value_counts('DEPARTURE_DELAY', sort=False)
        df2 = de.DataFrame({'DEPARTURE_DELAY': [-4.0, -1.0, 22.0, -3.0, 3.0, 21.0, -2.0],
                            'count': [2, 2, 2, 1, 1, 1, 1]})
        assert_frame_equal(df1, df2)

        df1 = df.value_counts('DEPARTURE_DELAY', dropna=False)
        df2 = de.DataFrame({'DEPARTURE_DELAY': [22.0, -1.0, -4.0, nan, -2.0, 21.0, 3.0, -3.0],
                            'count': [2, 2, 2, 2, 1, 1, 1, 1]})
        assert_frame_equal(df1, df2)

        df1 = df.value_counts('AIRLINE', dropna=False)
        df2 = de.DataFrame({'AIRLINE': [None, 'DL', 'AA', 'AS', 'WN', 'B6', 'UA', 'VX', 'EV'],
                            'count': [2, 2, 2, 1, 1, 1, 1, 1, 1]})
        assert_frame_equal(df1, df2)

        df1 = df.value_counts('DEPARTURE_DELAY', dropna=False, sort=False)
        df2 = de.DataFrame({'DEPARTURE_DELAY': [nan, -4.0, -1.0, 22.0, -3.0, 3.0, 21.0, -2.0],
                            'count': [2, 2, 2, 2, 1, 1, 1, 1]})
        assert_frame_equal(df1, df2)
